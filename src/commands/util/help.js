		...
		if(args.command && !showAll) {
			if(commands.length === 1) {
				...
				try {
					...
					if(msg.channel.type !== 'dm') messages.push(await msg.reply(`Sent you a DM containing more information on the command.`));
				} catch(err) {
			...
			try {
				messages.push(await msg.direct(stripIndents`
					...
					__**${showAll ? 'All commands' : `Available commands in ${msg.guild || 'this DM'}`}**__
							...
							**⚠️ YOU WILL ONLY SEE COMMANDS YOU HAVE PERMISSION TO ⚠️**
								**To see all commands do ${Command.usage('help all', msg.guild ? msg.guild.commandPrefix : null, this.client.user)}
						...
						.map(grp => stripIndents`
							...
							${grp.commands.filter(cmd => !cmd.hidden && (showAll || cmd.isUsable(msg)))
								.map(cmd => `\`${cmd.name}:\` *${cmd.description}<:Spartan:824723825597480978>*${cmd.nsfw ? ' (NSFW)' : ''}`).join('\n')
							}
						`).join('\n\n')
					}
				`, { split: true }));
				if(msg.channel.type !== 'dm') messages.push(await msg.reply(`Sent you a DM with all my commands.`));
			} catch(err) {
				messages.push(await msg.reply('Unable to send you the help DM. You probably have DMs disabled.'));
			}
			return messages;
