		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					...
					reply.setDescription(`>>> ❌ The \`${this.name}\` command requires you to have the "${permissions[missing[0]]}" permission.`)	
				}
				return;
			}
		}
	...
	onBlock(message, reason, data) {
		switch(reason) {
			...
			case 'permission': {
				...
				const missing = message.channel.permissionsFor(message.author).missing(this.userPermissions);
				const reply2 = new MessageEmbed()
				.setDescription(`>>> ❌ You do not have \`${permissions[missing[0]]}\` to use the \`${this.name}\` command.`)
				.setColor("RED")
				return message.channel.send(reply2).then(message =>{ message.delete({timeout: 10000})})
			}
			case 'clientPermissions': {
				if(data.missing.length === 1) {
					...
					.setColor("RED");
					message.channel.send(embedperms).then(message =>{ message.delete({timeout: 10000})})
				}
				return ;
			}
			case 'throttling': {
				const embed = new MessageEmbed()
				.setDescription(`>>> ⚠️ You may not use the \`${this.name}\` command again for another ${data.remaining.toFixed(1)} seconds.`)
				.setColor("YELLOW")
